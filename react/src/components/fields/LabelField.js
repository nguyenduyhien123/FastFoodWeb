...
export default function LabelField({ label, labelDir, fieldSize, option, type, placeholder, defaultOption, ...rest }) {
    return (
        <Box className={`mc-label-field-group ${label ? labelDir || "label-col" : ""}`}>
            ...
            {type ?
                ...
                <Select className={`mc-label-field-select ${fieldSize || "w-md h-sm"}`} {...rest}>
                    {defaultOption != null ? option?.map((item, index) => (
                        <Option key={index} selected={defaultOption === item?.id ? 'true' : 'false'} value={item?.id || item}>{item?.name || item?.name_vi || item}</Option>
                    )) :
