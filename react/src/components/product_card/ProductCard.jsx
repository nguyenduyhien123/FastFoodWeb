...
const ProductCard = (props) => {
  ...
  return (
    <section className="product_card h-100 card" data-aos="flip-left" data-aos-duration="1000">
      <div className="product_img text-center" onClick={handleProductDetailClick}>
        <img
          src={props.item.image?.startsWith('https') ? props.item.image : `http://localhost:8000/storage/uploads/${props.item.image}`}
          alt=""
          className="w-75"
        />
      </div>
      <div className="product_content">
        <div className="rating d-flex justify-content-center">
          <Rating size={20} rating={props.item.star} hoverColor="red" onRatingChanged={(e) => console.log(e)} />
        </div>
        <div className="d-flex justify-content-between gap-2 align-items-center mb-4">
          ...
          <Button className="material-icons favorite" onClick={() => handleAddToFavorite(props.item.id)}>
            favorite
          </Button>
        </div>
        <div className="d-flex align-items-center justify-content-between">
          <span className="price d-flex align-items-center">
            <span>{props.item.price.toLocaleString('vi')}</span>
            <sup>đ</sup>
          </span>
